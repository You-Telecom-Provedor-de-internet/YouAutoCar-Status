...
| **SHA status** | `8544a12` |
| **Data** | 2026-03-18 |
| **Modo** | EVOLUÇÃO DE PRODUTO — ONDA 7 OBD Knowledge Base |
| **build** | ✅ exit 0 (tsc --noEmit 0 erros) |
| **ONDA ativa** | ONDA 7 Fase 1 ✅ — RPCs fantasma criadas. Fase 2 pendente |
| **Próxima ação obrigatória** | Fase 2: regenerar tipos TS + testar dashboards no browser |

### Resumo da última rodada

**ONDA 7 Fase 1 — Criação das 5 RPCs fantasma do Knowledge Engine:**
- **5 RPCs criadas** no Supabase (migration + apply_migration):
  1. `get_knowledge_stats` — KPIs consolidados (evidências, hipóteses, DTCs, tags, veículos)
  2. `search_knowledge` — busca parametrizada por DTC, veículo, tag, tipo
  3. `search_knowledge_fulltext` — busca textual em título, summary, descrição, tags
  4. `ingest_knowledge_from_repairs` — importa reparos confirmados como evidências
  5. `get_cross_os_patterns` — padrões cross-OS (DTCs recorrentes, clusters de falha, reparos por modelo)
- **Tabelas-fonte**: `diagnostic_evidence`, `diagnostic_hypotheses`, `confirmed_repairs`, `vehicles`
- **Nenhuma tabela nova criada** — RPCs derivadas de tabelas existentes
- **CHECK constraint expandido**: `evidence_type` agora aceita `confirmed_repair_digest`
- **Segurança**: SECURITY DEFINER + `get_user_id()` + `company_members`
- **Dashboards desbloqueados**: `KnowledgeEngineDashboard` e `CrossOsPatternsDashboard` agora têm backend real
...
| Knowledge Engine | 🟢 | 🟢 | ONDA 4 ✅ + ONDA 7F1 ✅ — RPCs criadas no banco |
...
| Cross-OS Patterns | 🟢 | 🟢 | ONDA 4 ✅ + ONDA 7F1 ✅ — RPC criada no banco |
...
| EX-005 | `supabase.rpc('import_quotation_prices')` | quotationService | RPC transacional sem tipo gerado. | Apenas esta RPC específica. Remover quando tipos forem regenerados. |
...
| EX-009 | `supabase.storage` | diagnosticUploadService, evidenceService, diagnosticService | Storage nativo — queryService não cobre Storage. | Apenas operações: upload, createSignedUrl, remove. |
| EX-010 | `supabase.rpc('delete_diagnostic_entry')` | diagnosticService | RPC transacional sem tipo gerado. | Apenas esta RPC. Remover quando tipos forem regenerados. |
| EX-011 | `queryService.rpc as unknown as ...` | knowledgeEngineService | 4 RPCs sem tipo gerado. Cast via interface local. | Apenas estas 4 RPCs. Remover quando tipos forem regenerados. |
| EX-012 | `queryService.rpc as unknown as ...` | crossOsPatternsService | 1 RPC sem tipo gerado. Cast via interface local. | Apenas esta RPC. Remover quando tipos forem regenerados. |
...
#### 📋 Fase 2 — Regenerar tipos + testar dashboards

| Item | Status |
|------|--------|
| Regenerar `database.ts` via `generate_typescript_types` | 📋 Pendente |
| Remover casts `as unknown as ...` de `knowledgeEngineService.ts` | 📋 Pendente |
| Remover casts de `crossOsPatternsService.ts` | 📋 Pendente |
| Testar KnowledgeEngineDashboard no browser | 📋 Pendente |
| Testar CrossOsPatternsDashboard no browser | 📋 Pendente |

#### 📋 Fase 3+ — Decisões pendentes (Owner)

| Frente | Decisão necessária |
|---------|-------------------|
| CRUD `knowledge_rules` | Definir se será web-only ou web+mobile |
| Multi-tenant `knowledge_rules` | Decidir se regras são por company ou globais |
| Unificação `analisar-dtc` vs `dtc_analyze` | Auditar duplicação entre edge functions |
...
## ONDA 7 — OBD Knowledge Base em andamento

### Fase 1 ✅ CONCLUÍDA
- 5 RPCs fantasma criadas no banco (SHA `f1b6ce9`)
...
### Itens Owner (paralelos, não bloqueiam ONDA 7):
1. **Dependabot alerts** — 1 critical, 1 high, 1 moderate
2. **Edge Function `test-pdf`** — sem consumidores, aprovar remoção
